...
func main() {
	...
	// Part 1
	err := app.ProcessLines(input, func(l string) error {
		id, r, g, b := parseGameLogLine(l)
		if r <= 12 && g <= 13 && b <= 14 {
			answer += id
		}
		return nil
	...
	log.Println("The answer to part one is: ", answer)

	// Part 2
	answer = 0
	err = app.ProcessLines(input, func(l string) error {
		_, r, g, b := parseGameLogLine(l)
		answer += r * g * b
		return nil
	})
	if err != nil {
	...
	log.Println("The answer to part two is: ", answer)
}

func parseGameLogLine(l string) (id, r, g, b int) {
	parts := strings.Split(l, ":")
	id, err := strconv.Atoi(parts[0][5:])
	if err != nil {
		log.Fatalln("could not extract id", err)
	}
	parts = strings.Split(
		strings.ReplaceAll(parts[1][1:], ";", ","),
		", ",
	)
	for _, p := range parts {
		combo := strings.Split(p, " ")
		num, err := strconv.Atoi(combo[0])
		if err != nil {
			log.Fatalln("could not extract number from combo", err)
		}
		if combo[1] == "red" && num > r {
			r = num
		}
		if combo[1] == "green" && num > g {
			g = num
		}
		if combo[1] == "blue" && num > b {
			b = num
		}
	}
	return
